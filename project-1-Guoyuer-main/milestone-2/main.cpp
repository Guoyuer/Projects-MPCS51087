...
//    }
    ...
    v = atof(argv[6]);
    cout << "v = " << v << endl;
    int n_thread = omp_get_num_procs() - 1;

    if (argc == 8) {
        n_thread = atoi(argv[7]);
    }

    /* a double uses 8 bytes*/
    cout << "Estimated Memory Usage: " << N * N * 2 * 8 / 1024 / 1024 << "MB" << endl;

    cout << "Number of processors: " << n_thread << endl;
...
#pragma omp parallel for default(none) shared(Cn, denom, x0, y0, dx, dy, N)
    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= N; j++) {
            double x = (i - 1) * dx;
            double y = (j - 1) * dy;
...
#pragma omp parallel for default(none) shared(Cn, Cnn, N, dx, dt, u, v) num_threads(n_thread)
        for (int i = 1; i <= N; i++) {
            for (int j = 1; j <= N; j++) {
