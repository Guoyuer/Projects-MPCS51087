    ...
    */
    ...
    for (int n = 1; n <= NT; n++) {
        ...
        if(world_size > 1){
            ...
            for(int i = 1; i <= div; i++) {
                C[i][0] = C[i][div];//left
                C[i][div + 1] = Cn[i][1]; //right
                Cn[div + 1][i] = Cn[1][i]; //bottom
                Cn[0][i] = Cn[div][i]; //top
            }
        }
        ...
        for (int i = 1; i <= div; i++) {
            for (int j = 1; j <= div; j++) {
                Cn[i][j] = (C[i - 1][j] + C[i + 1][j] + C[i][j - 1] + C[i][j + 1]) / 4 -
                            dt * (u * (C[i + 1][j] - C[i - 1][j]) + v * (C[i][j + 1] - C[i][j - 1])) / (2 * dx);
            }
        }
